...
Single-page React app, no routing library.

- `src/index.js` — mounts `<App />` into `#root` (in `public/index.html`), wraps in `React.StrictMode`, calls `reportWebVitals()`.
- `src/App.js` — top-level layout. Holds the `activeTab` state and renders the hero/profile card, the three tab buttons, and the active tab's content section. **Tab content is hardcoded inline** (Projects is a manual enumeration of `<Project {...projectData.projectN} />` from `project1` through `project20`; Research iterates `researchData.papers`).
...
- `src/data/projectData.js` — exports `projectData` with keys `project1` … `project20`. **To add a project, add a new key here and append `<Project {...projectData.projectN} />` in `App.js`.** (There is no `.map()` over the data.)
...
- The portfolio data is in `src/data/`. The Projects tab is a **manual enumeration**, not a `.map()` — adding project N+1 requires both a new entry in `projectData.js` and a new `<Project ... />` line in `App.js`.
